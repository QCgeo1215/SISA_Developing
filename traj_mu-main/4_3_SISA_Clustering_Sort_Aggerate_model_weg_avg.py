...
def aggregate_predictions(test_npy_file, csv_path, final_models_A, final_models_B, batch_size, output_size, device, num_shards, num_slices, result_path, start_time):  
    ...
    #print("test_trj_id_set(len)",test_trj_id_set(len))
...
def main():
    ...
    for i in range(config['num_shards']):
        ...
        model_A.load_state_dict(torch.load(f"{full_path +'/4_1_SISA_Sort_lstm/SISA_final_model'}/final_model_shard-idx_{i+1}_shard-num_{config['num_shards']}.pth"))
        model_B.load_state_dict(torch.load(f"{full_path +'/4_1_SISA_Sort_lstm/shard_loss_model'}/final_loss_model_shard-idx_{i+1}_shard-num_{config['num_shards']}.pth"))
    ...
    aggregate_predictions(
        ...
        num_shards=config['num_shards'],
        num_slices=config['num_slices'],
        result_path=full_path +'/4_1_SISA_Sort_lstm/aggreate_result',
